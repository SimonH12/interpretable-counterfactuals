...
def prepare_adult_data() -> pd.DataFrame:
    ...
    # df = df.head(10) # comment this line out, gets just the first 100 rows just for trying things
    df = df.drop('educational-num', axis=1) # this is unnecessary already have the education level as a categorical value
    df = df.drop('capital-gain', axis=1) # MAD is zero (cant divide by zero)
    df = df.drop('capital-loss', axis=1) # MAD is zero too
...
def standardize_adult_data():
    ...
    df.to_csv(r"code\real_world_data\data\census.csv", index=False)
...
#     df_balanced.to_csv(r"datasets\adult_balanced.csv", index=False)



def balance_dating_data():
    df = pd.read_csv(r"datasets\dating_data_custom.csv")
    print("\ndf\n", df)
    ...
    df_balanced.to_csv(r"datasets\dating_balanced.csv", index=False)
...
def prepare_toy_data() -> pd.DataFrame:
    df = pd.read_csv(r"datasets\toy_dataset_copy.csv")
    return df
...
def create_adm_data():
    ...
    df = pd.read_csv(r"datasets\adm_data.csv")
    print("\ncolumns\n", df.columns)
    print("\ndf.info()\n", df.info())
    ...
    print("\ndf.head\n", df.tail(40))
    df.to_csv(r"code\real_world_data\data\admission.csv", index=False)


def get_adm_data_custom():
    df = pd.read_csv(r"datasets\adm_data_custom.csv")
    return df
...
def create_dating_data():
    ...
    df = pd.read_csv(r"code\datasets\Speed Dating Data.csv", encoding='cp1252')
    ...
    df.to_csv(r"datasets\dating_data_custom.csv", index=False)
...
def german_credit():
    ...
    df.to_csv(r"real_world_data\data\credit.csv", index=False)


def adult():
    df = helpers.load_adult_income_dataset()
    print("\ndf\n", df)
    df.to_csv(r"real_world_data\data\adult_dice.csv", index=False)
...
def wine():
    ...
    print("\ndfq\n", df)
    df.to_csv(r"real_world_data\data\wine.csv", index=False)
...
def unit_test_high_number_features():
    ...
    print("\ndf\n", df)
    df.to_csv(r"datasets\blobs_feats_ut.csv", index=False)
...
def unit_test_circles_close():
    ...
    df.to_csv(r"unit_test\out\circle_close.csv", index=False)
    return df
...
def unit_test_imbalanced(t=0):
    ...
    df_3.to_csv(r"unit_test\data\circles_imbalance_Bta_1000.csv", index=False)
    return df_3

def unit_test_circles():
    X, y = make_circles(n_samples=1000)
    df = pd.DataFrame(dict(x = X[:,0], y = X[:,1], label = y))
    df.to_csv(r"unit_tests\data\ut_circles.csv", index=False)
    return df
...
def unit_test_circles_noise():
    ...
    df = pd.DataFrame(dict(x = X[:,0], y = X[:,1], label = y))
    df.to_csv(r"unit_test\data\ut_circles_noise.csv", index=False)
    return df
...
def unit_test_circles_outlier():
    X, y = make_circles(n_samples=1000, factor=0.5, noise=0.1) # noise=0 or noise=0.1 works okay (works with genetic better than random)
    df = pd.DataFrame(dict(x = X[:,0], y = X[:,1], label = y))
    df.to_csv(r"unit_test\data\circles_outlier_noise.csv", index=False)
    return df
