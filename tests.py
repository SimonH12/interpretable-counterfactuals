...
class Unit_Tests():
    ...
    table_path = r"unit_tests\unit_test_table.csv"

    def __init__(self, target_class, target_class_name, name, path, ordering_features=False, knn_size=100, specific_inst=pd.DataFrame(), out=r"unit_tests\out", rounds=50, reps=1):
    ...
    def create_new_score(self):
        self.csv_file, self.scores = proxy.rounds_of_classifying(self.target_class, self.target_class_name, self.path, self.name, self.ordering_features, reps=self.reps, rounds=self.rounds, k=self.knn_size, inst=self.specific_inst, out_folder=self.out)
        print("\nscores\n", self.scores)
    ...
    def get_existing_scores(self):
        path = r"unit_tests\out" + '\\' + self.name + '.csv'
...
class Evaluation():
    name = 'Evaluation'
    ml_task = 'classification'
    table_path = r"real_world_data\real_world_table.csv"

    def __init__(self, target_class, target_class_name, name, path, ordering_features=False, knn_size=100, specific_inst=pd.DataFrame(), to_vary='all', svc=True, model="", rounds=10, reps=5, out=r"real_world_data\out"):
        self.target_class = target_class
        self.target_class_name = target_class_name
        self.name = name
        self.path = path
    ...
    def get_existing_scores(self):
        path = r"real_world_data\out" + '\\' + self.name + '.csv'
        isFile = os.path.isfile(path)
        if isFile:
            self.csv_file = path 
            df = pd.read_csv(path)
...
def do_all_unit_tests():
    ...
    path_circles = r"unit_tests\data\circles.csv"
    ...
    path_circles_noise = r"unit_tests\data\circles_noise.csv"
    ...
    path_circles_outlier = r"unit_tests\data\circles_outlier.csv"
    ...
    path_circles_imbalance_Bog = r"unit_tests\data\circles_imbalance_Bog.csv"
    ...
    path_circles_imbalance_Bta = r"unit_tests\data\circles_imbalance_Bta.csv"
    ...
    path_blobs = r"unit_tests\data\blobs_many_features.csv"
    ...
    path_circle_close = r"unit_tests\data\circle_close.csv"
...
def do_all_eval():
    ...
    path_adm = r"real_world_data\data\admission.csv"
    path_adm_model = r"real_world_data\model\model_adm.sav"
    ...
    path_credit = r"real_world_data\data\credit.csv"
    path_credit_model = r"real_world_data\model\model_credit.sav"
    ...
    path_wine = r"real_world_data\data\wine.csv"
    path_wine_model = r"real_world_data\model\model_wine.sav"
    ...
    path_adult = r"real_world_data\data\census.csv"
    path_adult_model = r"real_world_data\model\model_adult.sav"
